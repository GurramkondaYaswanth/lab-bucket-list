...
public class SetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashSet<TouristPlace> HSbucketlist = new HashSet<TouristPlace>();
		
		if(add!=null) {
			// call the add method and store the return value in a set variable
			
			request.setAttribute("bucketListadd", so.add(tp));
		...
		if(remove!=null) {
			...
			request.setAttribute("bucketListremove", so.remove(tp));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/set.jsp");
			rd.forward(request, response);
		}
		...
		if(sortbydestination!=null) {
			...
			Object set = so.sortByDestination(HSbucketlist);
		...
		if(sortbyrank!=null) {
			...
			Object set = so.sortByRank(HSbucketlist);
			request.setAttribute("bucketList", set);
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/set.jsp");
			rd.forward(request, response);
		}
		...
		if(reset!=null) {
			...
			Object set = so.reset(HSbucketlist);
			request.setAttribute("bucketList", set);
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/set.jsp");
			rd.forward(request, response);
		}
